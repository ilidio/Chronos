# IntelliJ-Style Local History & Git History

Bring the powerful history management of IntelliJ IDEA to Visual Studio Code. This extension provides a robust, non-Git local history (automatic snapshots) and a specialized Git history view for specific code selections.
...
### 1. Local History (The Safety Net)
The extension automatically starts tracking your files as soon as you save them.

- **View History:** Right-click anywhere in an open editor or on a file in the Explorer and select **Local History > Show History**.
...
1. Right-click and select **Local History > Put Label...**.
...
- **Local History:** Select a block of code, right-click -> **Local History > Show History for Selection**.
...
- Run the command `Local History: Show Project History` from the Command Palette (`Cmd+Shift+P`).

---

## ⚙️ Configuration

You can tune the extension in **File > Preferences > Settings** (search for `localHistory`):

- `localHistory.enabled`: Toggle the entire system.
- `localHistory.maxDays`: Automatically prune old history (default: 30 days).
- `localHistory.exclude`: Add folders like `build/` or `temp/` to keep your history storage clean.

## 🛠 Commands Reference

| Command | Description |
| --- | --- |
| `Local History: Show History` | Show timeline for the active file |
| `Local History: Put Label` | Create a named checkpoint |
| `Local History: Show Project History` | View changes across the whole workspace |
| `Local History: Git History for Selection` | Show Git commits for selected lines |
